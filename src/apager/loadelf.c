...
void * load_elf(FILE * file, void ** dest, void ** stack){
    ...
    if(!load_segment(file, ehdr, phdr, dest, stack)){
...
bool load_segment(FILE * file, Elf32_Ehdr * ehdr, Elf32_Phdr * phdr, void ** load_addr, void ** stack){
    ...
    *load_addr = mmap(NULL, length + PAGE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
    ...
    *stack = *load_addr + length + PAGE_SIZE;
    ...
    for(i = 0; i < ehdr -> e_phnum; i++){
        ...
        case PT_LOAD:
            ...
            fread(*load_addr + phdr[i].p_vaddr, 1, phdr[i].p_filesz, file);
            break; 
        }
    }
